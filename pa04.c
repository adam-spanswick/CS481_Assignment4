...
void *ride()
{
	...
	pthread_mutex_lock(&mutex);
		...
		int max = MAXPERCAR*numCars;

		// Calculate the number of people to send on the ride
		if(peopleInLine < max)
		{
		...
		else if(peopleInLine >= max)
		{
			passengers = max;
			peopleInLine = (peopleInLine - max);
